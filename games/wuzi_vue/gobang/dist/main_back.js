...
var Board = function(container, status) {
  ...
  // addde by jxc 20180715 
  /*
  this._set_outsideof_axios = function(x,y,role){
    self.set(x,y,role);
  }
  */
  // add end by jxc 
  ...
  this.container.on("click", function(e) {
    ...
    oReq.open("GET", "http://127.0.0.1:1337/get", false);  // 同步请求
    oReq.send(null);
    var res_withxy = JSON.parse(oReq.responseText);
    x = res_withxy.x;
    y = res_withxy.y;
    /*
    ...
    */
    ...
    var url_bridge = 'http://localhost:1337'
    re.open('GET',url_bridge,true);
    re.withCredentials = true;
    ...
    */    
    // 解决跨域问题使用了 cros 包 20180715    
    /*
    var that = self;
    axios({
      url: 'http://127.0.0.1:1337/get',
      method: 'GET',
      headers: {
...
// add end by jxc 20180714

Board.prototype._set = function(x, y, role) {
  // add by jxc 20180714
  // this.setStatus("role:  "+role);
  /*
  if(role == 1){
    role = 2
  }else{
  ...
  */
  ...
  axios({
      url: 'http://127.0.0.1:1337/put/?x='+x+'&y='+y,
      method: 'GET',
      headers: {
        'Content-Type': 'text/plain'
      }
